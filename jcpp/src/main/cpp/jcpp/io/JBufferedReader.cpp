...
namespace jcpp{
    namespace io{
        ...
        string JBufferedReader::readLine(jbool ignoreLF){
            ...
            for (;;) {
                ...
                if (eol) {
                    string str;
                    if (s == NULL) {
                        for (jint tmp=startChar;tmp<i-startChar;tmp++){
                            str.push_back(cb[tmp]);//TODO define static method
                        }
                    } else {
        ...
        string JBufferedReader::readLine(){
