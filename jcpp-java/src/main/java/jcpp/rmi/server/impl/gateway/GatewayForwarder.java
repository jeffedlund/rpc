...
public class GatewayForwarder implements Callable<Void> {
    ...
    public GatewayForwarder(Socket srcSocket, GatewayConfiguration gatewayConfiguration, GatewayTunnel tunnel) throws IOException, ClassNotFoundException {
        ...
        if ((tunnel != null) && !tunnel.getGateway().isTunnelOpener() && (nextAddress != null) && nextAddress.equals(tunnel.getTunnelAddress())) {
            ...
            try {
                ...
                if (log.isInfoEnabled()) {
                    log.info("new socket created: " + targetSocket);
                }
            } catch (IOException e) {
                // TODO remove catch
                e.printStackTrace();
