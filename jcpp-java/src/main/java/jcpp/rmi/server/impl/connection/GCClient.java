...
public class GCClient implements IGCClient {
    ...
    private class EndPointInfo implements Runnable {
        ...
        public void export(String id, Object object) {
            if (objects.size() == 0) {
                scheduledFuture = localServer.getScheduledExecutorService().scheduleAtFixedRate(this, localServer.getConnectionConfiguration().getGcClientTimeout(),
            ...
            try {
                ...
                if (++gcExceptionCount >= localServer.getConnectionConfiguration().getGcClientExceptionThreshold()) {
                    if (log.isWarnEnabled()) {
                        log.warn("Pinging the GC at " + remoteEndPoint + " failed after " + gcExceptionCount + " attempts. The remote endpoint will no longer be pinged. The last received exception follows", e);
                    }
                    if (log.isDebugEnabled()) {
        ...
        public String toString() {
            String[] strArray;
            synchronized (this) {
                strArray = objects.keySet().toArray(new String[0]);
            }
            String toString = "";
            if (strArray != null) {
            ...
            return "GCClient.EndPointInfo[RemoteEndPoint: " + remoteEndPoint + ", Ids of exportedObjects:" + toString + "]";
        }
